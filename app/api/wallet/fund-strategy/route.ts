...
import { createStrategyFundChallenge, getUserWallets } from "@/shared/lib/circle";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await createStrategyFundChallenge(userToken, wallet.id, registry, String(id), String(amount));
    ...
    const msg =
      ...
      ?? (err instanceof Error ? err.message : "Failed to fund strategy");
    console.error("[fund-strategy]", msg, err);
