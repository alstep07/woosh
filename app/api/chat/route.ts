...
Strategy setup: to create one you need the kind (recurring payment, auto-buy, or target allocation), amount per run, how often (daily/weekly/monthly), the recipient (for payments) or which token to buy (for auto-buy) or the allocation (for target allocation), and the total to deposit (funding, not needed for sweep mode). If the user gives a number of runs but not a total, compute funding = amount per run x runs. If you cannot determine the total, ask for it. As soon as you have everything, you MUST call create_strategy in that same turn, do not just say you will. To pause or cancel a recurring payment, auto-buy, or target allocation automation, guide the user to /dashboard/automations.`,
    },
